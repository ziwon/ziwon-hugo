...
> 공유 메모리로 통신하지 말라. 대신, 통신으로 메모리를 공유하라. (Do not communicate by sharing memory; instead, share memory by communicating)
...
현실적으로는 분명히 더 많은 것들이 있지만, 이 코드는 속도 제한, 병렬, 넌블락 RPC 시스템을 위한 프레임워크이다. 그리고 뮤텍스는 눈에 보이지 않는다.
